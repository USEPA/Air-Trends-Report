...
var infile = "site_quarterly_spec_withSiteName_2006-2015.csv";
...
var count = null;
var delimiter = '|';
var monitorFields = ['SiteID','State_code','County_code','Site_code','Latitude','Longitude','Site_name','City','State'];
...
function handleTextFileRow(row) {
  ...
  return Q.fcall(function () {
    row.SiteID = row.State_code + '-' + row.County_code + '-' + row.Site_code;
    var season = 'Q' + row.Quarter;
    if (! concentrations[row.SiteID]) {
      concentrations[row.SiteID]={};
    }

    if (! concentrations[row.SiteID][season]) {
      concentrations[row.SiteID][season] = [];
    }

    yearlyConcentrations = concentrations[row.SiteID][season];

    yearConcentrations = {Year:row.Year,Season:season};
  ...
  //  });
    ...
    concentrations[row.SiteID][season].push(yearConcentrations);
  //  yearlyConcentrations.push(yearConcentrations);

    //if monitor NOT in collection then add it
    if (! monitors[row.SiteID]) {
      //just add the monitor fields for each site code
  //    monitors[row.SiteID] = {};
  //    monitorFields.forEach(function (field) {
  //      monitors[row.SiteID][field] = row[field];
  //    });

      monitorsSiteIDs.push(row.SiteID);

      monitors[row.SiteID]= monitorFields.reduce(function (acc,val) {
        acc[val]=row[val];
        return acc;
      },{});
    }
...
//      console.log("data[quarter] " + data[quarter]);
      ...
      data[quarter].forEach(function (year) {
        if (! annual[year.Year]) annual[year.Year] = {};
        if (! annual[year.Year][species]) annual[year.Year][species] = [];

        if (year[species] && !isNaN(year[species])) annual[year.Year][species].push(year[species]);
      });
    });
  });
  data.Annual = Object.keys(annual).sort().map(function (year) {
    var yearData = {Year:year,Season:"Annual"};
