...
//    var translate = [d3.event.offsetX-self.percentSize*scale*d3.mouse(this)[0],d3.event.offsetY-self.percentSize*scale*d3.mouse(this)[1]];
//Need to calculate mouseRelativeToMapDiv. Before was using offsetX and offsetY but this was diff in IE vs Chrome.
//In chrome offsetX is distance between mouse and map div but in IE i think it was distance between mouse and scaled svg group which is not what is desired here
    var translate = [mouseRelativeToMapDiv.x-self.percentSize*scale*d3.mouse(this)[0],mouseRelativeToMapDiv.y-self.percentSize*scale*d3.mouse(this)[1]];
...
ClassifiedPointsMap.prototype.hidePointsWithMissingInfo = function () {
    ...
    self.points.style("display", function (d) {    
      ...
      return (hasLatLong && hasData) ? "" : "none";
    });
};
...
//return parent.append("g");
 ...
 //This for scaling the size of the icon based on what radius of point would be 
    ...
    .each(function(d) {
          var symbol = self.getSVGsymbol(d);
...
//    .attr("stroke-width",0 + "px");
 ...
 ClassifiedPointsMap.prototype.getSVGsymbol = function (d) {
  var self = this;
