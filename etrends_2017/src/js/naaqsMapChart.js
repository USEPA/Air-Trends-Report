...
etrends.naaqs.init = function () {
  var defaultMinYear = 1990;
	var yearRange = [defaultMinYear,2016];
...
//Set up the Map parameters for each pollutant  
  ...
  pollutantData.co = {file:"euninterp42101MAX2V19902016.csv",title:"CO 8-hour Concentration",standard:9,units:"ppm",digits:1,pollutantLabel:"CO"}
...
//Note used unicode characters to get subscripts for svg in charts  
  pollutantData.pb = {file:"euninterp14129RQMAX19902016.csv",title:"Lead 3-month Concentration",standard:.15,units:"\u03BCg/m\u00B3",digits:2,pollutantLabel:"Lead"}
...
//Note used unicode characters to get subscripts for svg in charts  
  pollutantData.no2_mean = {file:"euninterp42602AMEAN19902016.csv",title:"NO\u2082 Annual Concentration",standard:53,units:"ppb",digits:0,pollutantLabel:"NO\u2082 Annual"}
  ...
  pollutantData.no2_98 = {file:"euninterp42602P98V19902016.csv",title:"NO\u2082 1-hour Concentration",standard:100,units:"ppb",digits:0,pollutantLabel:"NO\u2082 1-hour"}
...
//no2 same for both types
  ...
  pollutantData.ozone = {file:"euninterp44201MAX4V19902016.csv",title:"Ozone 8-hour Concentration",standard:.07,units:"ppm",digits:3,pollutantLabel:"Ozone"}
  ...
  pollutantData.pm10 = {file:"euninterp81102MAX2V19902016.csv",title:"PM\u2081\u2080 24-hour Concentration",standard:150,units:"\u03BC g/m\u00B3 ",digits:0,pollutantLabel:"PM\u2081\u2080"}
...
//Once I make the chart titles HTML I can use this so that subscripts work in IE 11 
  pollutantData.pm25_mean = {file:"euninterp88101WTDAM20002016.csv",title:"PM\u2082.\u2085 Annual Concentration",standard:12,units:"\u03BCg/m\u00B3",digits:1,pollutantLabel:"PM\u2082.\u2085 Annual",minYear:2000}
  ...
  pollutantData.pm25_98 = {file:"euninterp88101P98V20002016.csv",title:"PM\u2082.\u2085 24-hour Concentration",standard:35,units:"\u03BCg/m\u00B3",digits:1,pollutantLabel:"PM\u2082.\u2085 24-hour",minYear:2000}
...
//pm 25 same for both types
  ...
  pollutantData.so2 = {file:"euninterp42401P99V19902016.csv",title:"SO\u2082 1-hour Concentration",standard:75,units:"ppb",digits:0,pollutantLabel:"SO\u2082"}
...
//Use the breaks for this pollutant
    ...
    filterAndPlotPoints(cacheData,pollutant);
...
//Now generate the summary stats for this pollutant if it doesn't exist
    if (! summaryStats[pollutant]) {
      ...
      var interpolatedData = statsCalculator.interpolateConcentrations(cacheData,function (row) {return row['nationalstats']=="Yes";});
...
//If not emissions then hide this div and exit
  ...
  d3.select("#naaqs-emissions-chart").style('display', hasEmissions ? '' : 'none');
  d3.select("#naaqs-emissions-chart-container .etrends-chart-legend").style('display', hasEmissions ? '' : 'none');  
  d3.select("#naaqs-emissions-chart-header").style('display', hasEmissions ? '' : 'none');  
  d3.select("#naaqs-emissions-chart-container #naaqs-emission-chart-alternateText").style('display', 'none');
  if (! hasEmissions) {
//for things like Lead with no emissions show a message
    if (pollutantData[pollutant].emissionsAlternateText) {
      d3.select("#naaqs-emission-chart-alternateText").style('display', '');
      d3.select("#naaqs-emission-chart-alternateText").html(pollutantData[pollutant].emissionsAlternateText);
      //Makes no sense that I try to set the title when there are no emissions graphs! remove later til figure out if this was just bad logic/copying
...
//filter out anything that is falsey BUT keep 0,"0.0","0" but NOT "". Note: ""==0 so have to do d.selectedSite!==""      
   ...
   self.makeChart();
    ...
    self.export.fileName = 'naaqs-concentrations-data_' + currentPollutant + '_' + selectedSiteData['aqs_site_id'];
...
//Draw the "sum lines" and then draw the total line because total line class passed
  ...
  function fitChartHeightToMapHeight() {
    ...
    if ($(window).width()<975) {
      ...
      emissionChart.overrideHeight = null;      
    } else {
      //f = emissionChart/concentrationChart after fitting to map height
      //If you want both charts equal height then f=1

      //f = emissionChart/(emissionChart+concentrationChart) after fitting to map height
      //If you want both charts equal then f=1/2
      //Could also use p = emissionChart/concentrationChart after fitting to map height
      ...
      concentrationChart.overrideHeight = (1-f)*(concentrationChart.chartHeight + emissionChart.chartHeight - extra);
      emissionChart.overrideHeight = f/(1-f)*concentrationChart.overrideHeight;

      //Makes no sense to redraw if height didn't change so only have it in this block
      concentrationChart.makeChart();
      if (pollutantData[currentPollutant].emissions.length>0) emissionChart.makeChart();
    }
  }  
