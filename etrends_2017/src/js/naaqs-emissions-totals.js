...
etrends.naaqs.emissions.totals.init = function (callback) {
  var self = this;
...
//Using hardcoded y ranges and interval for this
  ...
  var chartFileName = "data/naaqs/emissions/national_tier1_caps_emissTrends_lineChart_noWildfires.csv";
   ...
   var chart = new LineAreaChart("naaqs-emissions-totals-chart",125,.4,null); //passing the min height for the chart
...
//This variation is needed if chart div is relative so that tooltip calculation will use relative calculations
   ...
   chart.ytitle.text='Excluding CO'
...
//When moving line it must snap to integer year    
  ...
  args.done = function (cacheData) {
    ...
    //callback passed to init function that will fire when map is drawn
    //This is used when this chart on its own static page for testing because I have to make visible=true
    //When on actual report visible needs to be false until modal pops up
  ...
  chart.externalMakeChart = function () {   
      ...
      self.fitChartToScreen();      
  }

  this.fitChartToScreen = function() {
