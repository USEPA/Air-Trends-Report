...
etrends.visibility.init = function () {
...
//Now generate the summary stats for this type if it doesn't exist
    if (! summaryStats[type]) {
      ...
      var interpolatedData = statsCalculator.interpolateConcentrations(data,function (row) {return row['National_Trend_site']=="Yes";});
...
//filter out anything that is falsey BUT keep 0,"0.0","0" but NOT "". Note: ""==0 so have to do d.selectedSite!==""      
   ...
   chart.export.fileName = 'visibility-data_' + currentType + '_' + selectedSiteData['Siteid'];
