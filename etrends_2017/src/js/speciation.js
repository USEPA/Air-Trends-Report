...
etrends.speciation.init = function () {
...
//This variation is needed if map div is relative so that tooltip calculation will use relative calculations
  ...
  etrends.speciation.map.export.hardcodedDownloadPath = 'data/speciation/site_quarterly_spec_withSiteName_2006-2015.csv';
...
//have the range between min and max +/- some percent of range. Also make sure min is always >= 0
  ...
  //Now that data is retrieved either from file or cache assign chart.data and make chart
      chart.data = speciationConfig.concentrations.data[locationData.SiteID][currentSeason];
      chart.title.text = speciationConfig.title + "<br/><span class='etrends-chart-title-subtitle'>Site ID: " + locationData.SiteID + "</span>" || "nbsp;"; //If no site name use nbsp so just empty line at top with menu
      ...
      chart.export.fileName = 'speciationData_' + locationData.SiteID + '_' + currentSeason;
