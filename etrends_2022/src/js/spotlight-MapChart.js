...
//then stashing map/chart/slider stuff in etrends.spotlight[type]
if (!('etrends' in window)) window.etrends = {};
...
(function () {
  var config = {
    covid: {
      maxYear:91,
      startDate: '3/1/2020',
      initialPollutant: 'ozone',
      pollutantData: {
        ozone: {
          file:"spotlight_ozone.csv",
          fileDecade:"spotlight_ozone_10yrstats.csv",
          title:"Ozone 8-hour Concentration",
          standard:.07,units:"ppm",digits:3,
          pollutantLabel:"Ozone",usedInNatAggRange:[0,91],
          breaks: [.04,.054,.070],
          truncatedDecimals: 3
        },
        no2: {
          file:"spotlight_no2.csv",
          fileDecade:"spotlight_no2_10yrstats.csv",
          title:"NO\u2082 1-hour Concentration",
          standard:100,units:"ppb",digits:0,
//Note used unicode characters to get subscripts for svg in charts  
          pollutantLabel:"NO\u2082 Annual",usedInNatAggRange:[0,91],
          breaks: [20,53,100],
          truncatedDecimals: 0
        }
      }
    ...
    wildfires: {
      ...
      pollutantData: {
        pm25: {
          file:"spotlight_pm25.csv",
          fileDecade:"spotlight_pm25_10yrstats.csv",
          title:"PM\u2082.\u2085 24-hour Concentration",
          standard:35,units:"\u03BCg/m\u00B3",digits:1,
          pollutantLabel:"PM\u2082.\u2085 24-hour",
          usedInNatAggRange:[0,61],
          breaks: [12,35.4,55.4],          
          truncatedDecimals: 1
        },
      }
...
etrends.spotlight.init = function (D3QueueCallback,type) {
  if (! etrends.spotlight[type]) etrends.spotlight[type] = {};
  if (! etrends.spotlight[type].concentrations) etrends.spotlight[type].concentrations = {};

  var defaultMinYear = 0;
	var yearRange = [defaultMinYear,config[type].maxYear];
  var startDate = config[type].startDate;
  ...
  var xTickSize = 20;
  var pollutantDropdown = d3.select("#spotlight-" + type + "-pollutant-dropdown").node(); 
  var currentPollutant = pollutantDropdown.options[pollutantDropdown.selectedIndex].value || config[type].initialPollutant;
...
//Set up the Map parameters for each pollutant  
  var pollutantData = config[type].pollutantData;

  var naaqsMap = new ClassifiedPointsMap("spotlight-" + type + "-map");
  naaqsMap.hideEmptyActiveAttributes = true;
  naaqsMap.disableMapLegendAutoScaling = true;

  naaqsMap.pointSize=9;
  naaqsMap.namespace = "etrends.spotlight." + type + ".concentrations.map";
...
//resize charts after map resizes so that we can fit chart heights to map height
  ...
  naaqsMap.export.fileName = 'Spotlight-' + type + '-Map-' + yearRange[0];
...
//Add custom control to map export menu
  naaqsMap.export.tooltip.customRender = function () {
    var templateElement = $("#spotlight-" + type + "-map-menu-tooltip-template-customControls");
    var template = "";
...
//Note: need to pass the chart custom tooltip class because there are 2 tooltips on chart (another on legend)
   var concentrationChart = new LineAreaChart("spotlight-" + type + "-concentrations-chart",125,.25,null,"#spotlight-" + type + "-concentrations-chart-tooltip"); //passing the min height for the chart; 
    ...
    max = Math.max(max,d3.max(data, function (d) { return 1*d.p90; }))
     ...
     if (concentrationDecadeStore) {
      min = Math.min(min,d3.min(data, function (d) { 
        return 1*concentrationDecadeStore[d.year].p10; }));
      max = Math.max(max,d3.max(data, function (d) { 
        return 1*concentrationDecadeStore[d.year].p90; }));
     }
...
//  concentrationChart.addLegend = concentrationChart_addLegend;
  ...
  concentrationChart.namespace = "etrends.spotlight." + type + ".concentrations.chart";

  concentrationChart.addSeriesField('name',['decadeHighLow','decadeAverage','percentile','average','selectedSite']);
  concentrationChart.addSeriesField('text',['90%ile - 10%ile ','Average (2010-2019)','90%ile - 10%ile (2020)','Average (2020)','Selected Site (2020)']);
...
//Use this if the columns or column names are different
  ...
  concentrationChart.export.fileName = 'spotlight-' + type + '-concentrations-data_' + currentPollutant;
...
//Make full steps don't interpolate between values
  ...
  var naaqsSlider = new MapChartSlider("spotlight-" + type + "-Slider",yearRange[0],yearRange[1],1,1,1.33,xTickSize,onSlide,concentrationChart.tickFormat);
...
//only change map if full year not in between ticks
    if (value==parseInt(value)) {
      ...
      d3.select('#spotlight-' + type + '-Slider #sliderText').text(day);
...
//        naaqsMap.changeActiveAttribute("day" + value);
      ...
      naaqsMap.export.fileName = 'Spotlight-' + type + '-Map-' + value;
    ...
    args.makeLayers = function (callback) {
      ...
      if (!preLoadMaps) {
        makeConcentrationLayer(pollutant,callback);
          ...
          d3.select("#spotlight-" + type + "-container").style("pointer-events","none");
          d3.select("#spotlight-" + type + "-container").style("opacity",.3);
        ...
        .then(function () {
          d3.select("#spotlight-" + type + "-container").style("pointer-events","");
          d3.select("#spotlight-" + type + "-container").style("opacity",1);
          preLoadMaps = false;
          callback(null);
        })
      }
...
//  setMapTitle(pollutant);
  etrends.spotlight[type].concentrations.map.title.draw(pollutantData[currentPollutant].title || "nbsp;");
  etrends.spotlight[type].concentrations.map.export.hardcodedDownloadPath = "data/spotlight/concentrations/" + pollutantData[currentPollutant].file;
  etrends.spotlight[type].concentrations.chart.export.fileName = 'spotlight-' + type + '-concentrations-data_' + currentPollutant;
  //Check if we need to add a minium to national stats eg. Lead
    if (pollutantData[currentPollutant].usedInNatAggRange) {
      etrends.spotlight[type].concentrations.chart.dragBarMin=pollutantData[currentPollutant].usedInNatAggRange[0]; 
    } else {
      etrends.spotlight[type].concentrations.chart.dragBarMin=null;
    }      

  //If changing limit of dragBar eg. Lead then change start of slider
  if (pollutantData[pollutant].usedInNatAggRange && naaqsSlider.currentValue<etrends.spotlight[type].concentrations.chart.dragBarMin) naaqsSlider.currentValue=etrends.spotlight[type].concentrations.chart.dragBarMin;
...
//hide the open tooltip when the pollutant switches
  etrends.spotlight[type].concentrations.map.closeToolTip();
...
function setMapTitle() {
    var mapTitle = d3.select("#spotlight-" + type + "-map-title-label");
    ...
    var available = $("#spotlight-" + type + "-map-title").width() - 3.5 * $("#spotlight-" + type + "-map-home").width();
    var scale = Math.max(1,$("#spotlight-" + type + "-map-title-label").width() / available);
...
//filter out anything that is falsey BUT keep 0,"0.0","0" but NOT "". Note: ""==0 so have to do d.selectedSite!==""      
   ...
   self.makeChart();
    ...
    self.export.fileName = 'spotlight-' + type + '-concentrations-data_' + currentPollutant + '_' + selectedSiteData['site_id'];
...
function naaqsMap_externalMouseOut(d) {
  ...
  //Clear out site ID now that site is not on chart
      self.export.fileName = 'spotlight-' + type + '-concentrations-data_' + currentPollutant;
  },0);
}
    ...
    if (concentrationDecadeStore) {
      ...
      //    .interpolate('basis')
          .x (function (d) { 
            return self.xScale(d.year); })
          .y0(function (d) { 
            return self.yScale(concentrationDecadeStore[d.year].p90); })
          .y1(function (d) { 
            return self.yScale(concentrationDecadeStore[d.year].p10); });
...
function concentrationChart_externalAddLegend() {
  ...
  var legPercentileTip = new MapChartTooltip(self.host,"#spotlight-" + type + "-concentrations-chart-percentile-tooltip");  
...
//                concentrationChart.showToolTip();
  ...
  function fitChartHeightToMapHeight() {
    ...
    if ($(window).width()<975) {
      ...
      var extra = $("#spotlight-" + type + "-charts-container").outerHeight() - $("#" + naaqsMap.containerID).outerHeight();
...
//Here I'll expose some stuff to the outside. probably could have attahced everything in here to etrends.spotlight[type] object if neccesary but this should work for now.
//But turns out made re using naaqs for the spotlight stuff a bit easier!
  etrends.spotlight[type].concentrations.switchPollutant=switchPollutant;
  etrends.spotlight[type].concentrations.fitChartHeightToMapHeight=fitChartHeightToMapHeight;
  etrends.spotlight[type].concentrations.toggleAllSites=toggleAllSites;
  etrends.spotlight[type].concentrations.map=naaqsMap;
  etrends.spotlight[type].concentrations.chart=concentrationChart;
};
})();

//# sourceURL=spotlight-MapChart.js