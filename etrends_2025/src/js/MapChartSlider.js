 var MapChartSlider = function (domID,min,max,step,stepIncrements,stepTime,tickSize,onSlide,tickFormat,initalValue) {
  var self = this;
  this.domID=domID;
  ...
  this.currentValue = initalValue || initalValue===0 ? initalValue : min;
...
//set up resopnsiveness when resized
  this.debouncer = new MapChartResize(this, function() {
    //When resizing maintain current value
    var currentValue = self.slider.value();
    self.refresh();
...
//  var axis = d3.svg.axis().orient("bottom").tickFormat(d3.format("d"));
  var tickFormat = self.tickFormat || d3.format("d");
  var axis = d3.svg.axis().orient("bottom").tickFormat(tickFormat);
