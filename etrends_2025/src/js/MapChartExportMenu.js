...
etrends.library.MapChartExportMenu.prototype.downloadFile = function (fileName,options) {  
  //use this function now so that only IE uses download lib
  return etrends.library.utilities.downloadURL(fileName);
...
etrends.library.MapChartExportMenu.prototype.exportCSV = function (fileName,options) {  
  ...
  if (this.hardcodedDownloadPath) {
    etrends.library.utilities.downloadURL({url:this.hardcodedDownloadPath,fileName:fileName});
    return;   
  }
...
//This is custom function to getData for each host type (chart vs map)
  ...
  var downloadArgs = {
//pass blob instead of url and url will be created from blob    
    blob:formBlob,
    fileName: fileName || 'ExportedData.csv',
    //Note: type only needs to be passed if using IE
    type: type
  }

  etrends.library.utilities.downloadURL(downloadArgs);
...
etrends.library.MapChartExportMenu.prototype.print = function (title,width) {  
  ...
  html += '</head><body style="height:auto">';
  html += '<img />';  
  html += '</body><html>';
...
//    return self.addImageToCanvas(context,item.src,0,lastResult.y)
  ...
  return etrends.library.utilities.loopDeferSeries(imageSRCs,callback,{y:margin})
    .then(function () {
      ...
      if (filename) {
        //Note: type only needs to be passed if using IE
        etrends.library.utilities.downloadURL({url: canvasdata,fileName: filename,type: "image/" + type});
      } 
...
//something changed in chrome and setting width/height attr on svg wasn't changing  .height() or .width() property 
//just pass back the height and width that attr originally set to 
//Note: make sure to convert text to number
//  var obj = {src:imgsrc,width:svgClone.width(),height:svgClone.height(),svgText:svgText,isIE:isIE}; 
  var obj = {src:imgsrc,width:+svgClone.attr("width"),height:+svgClone.attr("height"),svgText:svgText,isIE:isIE}; 
...
if (etrends.library.utilities) etrends.library.utilities.setModuleLoadedForEGAM('MapChartExportMenu.js');

//# sourceURL=etrends/MapChartExportMenu.js
