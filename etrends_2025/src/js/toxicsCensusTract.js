...
var EsriModules = {};
etrends.toxicsCensusTract.EsriModules = EsriModules;
...
//Matt Woody only want it up to 100 per million
  ...
  var zoomedOutLayer = {
//For the tile layer we can't select (or just don't know how to yet?)    
    onSelect: onSelect,
    onZoom: zoomedOutLayerOnZoom,
//    onHitTest: onHitTest,
    service: "Cancer_Risk_2020",
    serviceType: 'MapServer',
    //This is the feature service corresponding to this tile service
    //We need to know this so we can hit feature to get info when clicking tile
    relatedFeatureServiceIndex: 1,
    //This because when zooming out if we try to refrsh highlights it takes for every to query server to get highlights so don't do it
    //Actually Jason fixed the service block ID index I guess because this is returning fast now...
    highlightZoomThreshold: 0,
    layerClass: EsriModules.TileLayer,
    highlight: true,
    layerArgs: {
//      url: 'https://services.arcgis.com/cJ9YHowT8TU7DUyn/ArcGIS/rest/services/Cancer_Risk_(2020)/MapServer/',
      definitionExpression:definitionExpression,
      //Note: The scale value used in minScale and maxScale is equal to view.scale ie. scale in 1:scale
      //But when coming up with scale range governed by min and max scale the range is 1/minScale to 1/maxScale
      //So minScale is bigger than maxScale because it is inverted
      //Kind of wacky presentation
      //For scale min=infinity to max=9000001 range would be 0 to 1/9000001
      maxScale: 100001 
    },    
//    defaultClass: defaultClass,
// Actually don't need default class = null now to this since there are no zero population polygons in 2024 report 
// Note: by default the zero populatoin break is added unless you set defaultClass=null on the layer
    defaultClass: null,
    symbol: defaultClass.symbol
  };
   function zoomedOutLayerOnZoom(layerInfo,mapInfo) {
    //note: minScale and maxScale are inverted values so scale must be greater than max and less than min to be in range
    var currentScale = mapInfo.view.scale;
    if (currentScale>=layerInfo.layerArgs.maxScale) {
      console.log('zoomed out layer showing');
      //don't need any args other than results because just trying to clear out the selection
      /* Don't need to clear selection anymore because I figured out how to click on tile layer and select
      var args = {
        layerInfo: layerInfo,
        results:{features:[]},
        attributes: null
...
//    onHitTest: onHitTest,
    service: "Cancer_Risk_2020",
    layerArgs: {
//      url: 'https://services.arcgis.com/cJ9YHowT8TU7DUyn/ArcGIS/rest/services/Cancer_Risk_2020/FeatureServer/',
      definitionExpression:definitionExpression,
      //Note: The scale value used in minScale and maxScale is equal to view.scale ie. scale in 1:scale
      //But when coming up with scale range governed by min and max scale the range is 1/minScale to 1/maxScale
      //So minScale is bigger than maxScale because it is inverted
      //Kind of wacky presentation
      //For scale min=9000000 to max=0 range would be 1/9000000 to infinity
      minScale:100000 
    },
  ...
  */
  ...
  //  NATA19AC_CRP_CRSG
  //GPO items for Cancer Risk 2020 from Jason Smith
  //Cancer Risk tile layer: https://epa.maps.arcgis.com/home/item.html?id=fce3e286c5054e54b9c320b717093a1b
  //Cancer Risk feature layer: https://epa.maps.arcgis.com/home/item.html?id=b9f3b4d74a0248d9becbfe267f356b3f
...
//      service: "Cancer_Risk_2020",
//Map Service     
//      service: "Cancer_Risk_(2020)",      
//      service: "Cancer_Risk_2020",      
//  Doug Solos
//      service: "Tract_Changes_2018",       
//      key: "FIPS",
// 2020 key is Block ID
      key: "Block_ID",
      layerArgs: {
...
//This is direct to Jason Smith's 2020 new service
//          url: 'https://services.arcgis.com/cJ9YHowT8TU7DUyn/ArcGIS/rest/services/Cancer_Risk_(2020)/MapServer/',
...
//For 2020 layer is 0
  ...
  var types = {
  /* 2019 Fields
  Total: {gpoField:'Total_Cancer_Risk',fullName:'Total Cancer Risk',breaks: totalBreaks}
  */
  /* 2020 Fields
  */
  Total: {gpoField:'Total_Cancer_Risk__per_million_',fullName:'Total Cancer Risk',breaks: totalBreaks}
}
  ...
  */
  var gpoFields = {
    ...
    Benzene: {gpoField:'BENZENE'},
    Cadmium: {gpoField:'CADMIUM_COMPOUNDS',breaksScale2:.1},
    Coke: {gpoField:'COKE_OVEN_EMISSIONS',breaksScale2:.1},
    Chloroprene: {gpoField:'CHLOROPRENE',breaksScale2:.01},
    Chromium: {gpoField:'CHROMIUM_VI__HEXAVALENT_'},
    Ethylene: {gpoField:'ETHYLENE_OXIDE'},
    Formaldehyde: {gpoField:'FORMALDEHYDE',breaksScale2:10},
    Nickel: {gpoField:'NICKEL_COMPOUNDS',breaksScale2:.1}
  };
  /* 2019 Fields
  var gpoFields = {
...
//2020 key is Block ID and don't use Risk Change anymore. Need to figure out where Popup_Note will come from for 2020
//in 2019 censut tract 17089852700 (Kane County, IL near Batavia) has a Popup_Note
    outFields: ["Block_ID","County_FIPS_Code","County_Name","State","Caveat_Notes"],
...
// 2020 Key is now Block ID
    $fips.text(args.attributes.Block_ID);
    $county.show();
//    $county.html(args.attributes.County_Nam + ', ' + args.attributes.State + '<br/>');
// 2020 County_Name field corrected to not be Nam
    $county.html(args.attributes.County_Name + ', ' + args.attributes.State + '<br/>');

//Example of block with caveat is shwon in this doc
//Air-Trends-Report-Dev\docs\Maps Showing Location of Toxic Census Tract block with Caveat.pdf    
//    var caveat_text = args.attributes.Popup_Note;
//2024 caveat field is now called 
    var caveat_text = args.attributes.Caveat_Notes;
    if (caveat_text) {
      $caveat.show();
...
//article on changing basemaps
// https://www.esri.com/arcgis-blog/products/js-api-arcgis/developers/how-to-use-a-basemap-with-a-specific-world-view-in-your-web-app    
//group with GOA basemaps
// https://www.arcgis.com/home/group.html?sortField=modified&sortOrder=desc&id=68efc5ce1e0b43cb819f7bba4a6bd7ea#content
    
    const grayBasemap = new EsriModules.Basemap({
      portalItem: {
      id: "c5b0e6e1ded840639b8b4ff3d9927145"  // light gray Vector webmap with GOA
      }
    });
...
//Have to use the GOA basemap
    ...
    view.watch("stationary", function (stationary) {
      mapInfo.layersInfo.forEach(function (layerInfo,index) {
        ...
        if (ready) watchViewLayerReady(layerInfo,mapInfo);
      });  
    });
    ...
    //          outFields: outFields
      ...
      mapInfo.layersInfo.forEach(function (layerInfo,index) {
        ...
        if (layerInfo.onSelect) {
          ...
          var featurObjectToQuery;
          var featureObjectFromServer=false;
          var featurObjectToHitTest;
          if (layerInfo.serviceType==='MapServer') {
            if ('relatedFeatureServiceIndex' in layerInfo) {
              featureObjectFromServer = true;
              //when clicking map server with related feature server need to hit layer not layer view so that we hit server not what is loaded in browser
              featurObjectToQuery = mapInfo.layers[layerInfo.relatedFeatureServiceIndex];
              featurObjectToHitTest = featurObjectToQuery;
              //need to add the outFields param when hitting the server
              //get outFields on current feature layer object
              params.outFields = featurObjectToQuery.outFields;
              //Also need to return geometry when hitting server from tile layer so we can highlight
              params.returnGeometry = true;  
            }        
          } else {
            featurObjectToQuery = view.layerViews.items[index];
            featurObjectToHitTest = mapInfo.layers[index];
        }          

          if (!featurObjectToQuery) {
             args.results = {features:[]};
             args.attributes = null;
            layerInfo.onSelect(args);  
          } else {
            featurObjectToQuery.queryFeatures(params)
            .then(function (results) {
          //        console.log(results);
              args.results = results;
              if (results.features.length) {
                var feature = results.features[0];
                args.attributes = feature.attributes
                if (featureObjectFromServer) {
        ...
        if (layerInfo.onHitTest || layerInfo.highlight!==false) {
          ...
          view.hitTest(screenPoint,{include:featurObjectToHitTest})
          .then(function (response) {  
...
//      console.log('clearHighlights' + layerInfo.layerIndex);
      */
      ...
      //Also don't try to create highlight if highlight is false for the layer
      if (layerScaleInView(layerInfo,mapInfo.view.scale) && layerInfo.highlight!==false) {
...
//        console.log(layerInfo);
 ...
 //       console.log(params);
        ...
        var featurObjectToQuery;
        if (layerInfo.serviceType==='MapServer') {
          //highlightZoomThreshold set so we don't try to highlight tiny polygons that we can't see
          //those turn out to take AGS long time to come back and not needed. should be way to query AGS by blockID only that is fast but can't figure it out
          if (mapInfo.view.zoom >= (layerInfo.highlightZoomThreshold || 0) && 'relatedFeatureServiceIndex' in layerInfo) {
            //when clicking map server with related feature server need to hit layer not layer view so that we hit server not what is loaded in browser
            //also need geometry to intersect with view extent
            //this was to slow when zoomed out  
            params.geometry = view.extent;
            params.spatialRelationship = 'esriSpatialRelContains';
//            params.spatialRel = 'esriSpatialRelEnvelopeIntersects';
            
            featurObjectToQuery = mapInfo.layers[layerInfo.relatedFeatureServiceIndex];
            //need all the parameters so we can get proper key value when highlighting feature
            params.outFields = featurObjectToQuery.outFields;

          }
        } else {
          featurObjectToQuery = layerInfo.layer_view;
        }
 ...
 //       console.log('layer scale not in view: ' + layerInfo.layerIndex);
    ...
    function watchViewLayerReady(layerInfo,mapInfo) {
        if (layerInfo.onZoom) layerInfo.onZoom(layerInfo,mapInfo);
        refreshHighlights(layerInfo,mapInfo);
    }
...
//        layerInfo.highlightGraphics = new EsriModules.Collection();
        ...
        if (!layerArgs.source && !layerArgs.url) {
          var serviceType = layerInfo.serviceType || 'FeatureServer'
          layerArgs.url = 'https://services.arcgis.com/cJ9YHowT8TU7DUyn/arcgis/rest/services/' + layerInfo.service + '/' + serviceType + '/';
        }
      ...
      if (typeInfo.defaultClass===null) {
        ...
        if (layerInfo.defaultClass===null && !typeInfo.defaultClass) {
          ...
          if (baseDefaultClass) {
            ...
            defaultClass = $.extend(true,{},typeInfo.defaultClass);
          }
        }
      }
...
//      delete args.defaultLabel;
//      delete args.defaultSymbol;
