...
//  config.symbology = {colors:[d3.rgb(0,80,81)],breaks:[]};
  ...
  config.types = {};
	config.types.impaired = {file:"impaired_2000-2024.csv",title:"Visibility Trend on Most Impaired Days",units:"deciview",digits:1,typeLabel:"Most Impaired",tooltip:"Click to view visibility trends on 20 percent most impaired days"}
	config.types.clearest = {file:"clearest_2000-2024.csv",title:"Visibility Trend on Clearest Days",units:"deciview",digits:1,typeLabel:"Clearest",tooltip:"Click to view visibility trends on 20 percent clearest days"}
...
//This variation is needed if map div is relative so that tooltip calculation will use relative calculations
  ...
  map.export.tooltip.customRender = function () {
    var templateElement = $("#visibility-map-menu-tooltip-template-customControls");
    var template = "";
    if (templateElement.length>0) template = templateElement.text();
    map.export.tooltip.div.select(".customControls").html(template);
    $(".visibility-map-menu-tooltip-showAllSites").attr("checked",showAllSites);
  };
...
//setting maxTickCount will figure out the tick values automatically so too many wont show up axis.
//works way better than setting xTickSize
    chart.maxTickCount = maxTickCount;
...
//          data = data.slice(0,10);
    ...
    //this will plot the points with the ability to switch between all points and just those in nat agg
    filterAndPlotPoints(data,type);
...
function filterAndPlotPoints(cacheData,type) {
...
//    var saveSymbols = config.symbology.symbols;
...
//    config.symbology.symbols = saveSymbols;
    ...
    console.log('done with visibility type ' + type);  
}
//A way to toggle 
function toggleAllSites(showAll) {
  ...
  setTimeout(function () {
    ...
    map.pointsCache = {};
    var cacheData = dataCache[currentType];

    //plot the points after filtering out non stats sites if not showing all
    filterAndPlotPoints(cacheData,currentType);
  },0);
}
...
//Here I'll expose some stuff to the outside. probably could have attahced everything in here to etrends.naaqs  object if neccesary but this should work for now.
  ...
  etrends.visibility.toggleAllSites = toggleAllSites
