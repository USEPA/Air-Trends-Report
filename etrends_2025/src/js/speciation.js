...
//speciationConfig.data is cache for storing monitors now that we switch between all and complete_sites only
  speciationConfig = {file:"data/speciation/monitors.json",title:"PM\u2082.\u2085 Composition Trend",data:{}
//concentrations is place to store concentration stuff. data stored in data for caching and files in folder
  ...
  speciationConfig.showAllSites = false;
...
//This variation is needed if map div is relative so that tooltip calculation will use relative calculations
  ...
  etrends.speciation.map.export.tooltip.customRender = function () {
    var templateElement = $("#speciation-map-menu-tooltip-template-customControls");
    var template = "";
    if (templateElement.length>0) template = templateElement.text();
    etrends.speciation.map.export.tooltip.div.select(".customControls").html(template);
    $(".speciation-map-menu-tooltip-showAllSites").attr("checked",speciationConfig.showAllSites);
  };
...
//  etrends.speciation.map.export.hardcodedDownloadPath = 'data/speciation/site_quarterly_spec_withSiteName_2000-2023.csv';
  etrends.speciation.map.export.hardcodedDownloadPath = 'data/speciation/site_quarterly_spec_withSiteName_2000-2023_wcompleteness.csv';
...
//setting maxTickCount will figure out the tick values automatically so too many wont show up axis.
//works way better than setting xTickSize
  etrends.speciation.chart.maxTickCount = 7;
...
//          data = data.slice(0,10);
        ...
        filterAndPlotPoints(data)
    };
    args.cache = speciationConfig.data;
    args.key = 'monitors';
    etrends.library.utilities.loadMapOrGraph(args,callback);
  }

    function filterAndPlotPoints(cacheData) {
//If showAllSites is not true then fiilter out sites that aren't used in national stats
        var data;
        //this key is used for plot points cache
        var key;
        if (speciationConfig.showAllSites) {
            key = 'all';
            data = cacheData;
        } else {
            key = 'complete';
            //var startDate = new Date();
            data = cacheData.filter(function (row) {
                return row['complete_site']==='Y';
            });
            //var endDate = new Date();
            //console.log("Filter Data Elapsed ms: " + (startDate.getTime()-endDate.getTime()));
        }
        //Format complete site label since file only says Y or N
        data.forEach(function (row) {
            row.f_complete_site = row.complete_site==='Y' ? 'Yes' : 'No';
        });

//pass type now so that map layers are cached for each type to speed up rendering
        var startDate = new Date();

        etrends.speciation.map.plotPoints(data,speciationConfig.symbology,key);

        var endDate = new Date();
        console.log("Plot Points Elapsed ms: " + (startDate.getTime()-endDate.getTime()));
        console.log('done with speciation monitors key' + key);
    }
    ...
    function toggleAllSites(showAll) {
        ...
        setTimeout(function () {
            ...
            speciationConfig.showAllSites = showAll;
            var cacheData = speciationConfig.data.monitors;

            //plot the points after filtering out non stats sites if not showing all
            filterAndPlotPoints(cacheData);
        },0);
    }
  ...
  etrends.speciation.toggleAllSites = toggleAllSites;
