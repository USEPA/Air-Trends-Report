...
//  if (! this.getXdomain) this.getXdomain =  function (self) {var ex=d3.extent(self.data, function (d) { return 1*d[self.xField]; });ex[1]=2015;return ex};
  ...
  var xTickValues;
  if (this.xTickValues) {
    xTickValues = this.xTickValues;
  } else {
    //Given a max tick count, this figures out how which tick values to use
    if (this.maxTickCount) {
  // xTickCount = (xMax - xMin)/xTickWidth + 1; 
  // we can then find xTickCount if xTickwidth = 1 minimum value
  // xTickCount (xTickWidth=1) = (xMax - xMin) + 1   
  // Check where xTickCount (xTickWidth=1) <= maxTickCount
  // If no then have to recalculate xTickWidth using xTickCount=maxTickCount
  // xTickWidth = (xMax - xMin)/(maxTickCount-1)

  //Note: use xDomainDefault instead of xDomain because sometimes for say bar charts x domain off by +/- .5 years to add padding for labels
        var xDomainDefault = this.getXdomainDefault(this);
        var xDomainStart = xDomainDefault[0];
        var xDomainEnd = xDomainDefault[1];
        var xDomainWidth = xDomainEnd - xDomainStart;
        var xTickWidth = 1;
        var xTickCount = xDomainWidth+1;
        if (xTickCount > this.maxTickCount) {
            xTickCount = this.maxTickCount;
            xTickWidth = Math.ceil(xDomainWidth/(this.maxTickCount-1));
        }
        xTickValues = this.generateTickValues(xDomainStart,xDomainEnd,xTickWidth);
    }
  }
...
LineAreaChart.prototype.drawBars = function(spacing,maxBarCount) {
   ...
   //Get the group of bars first and attach data to it
    ...
    //After that actually use y at zero
      if (seriesIndex==1) yAtZero = self.yScale(0);
...
//      .attr('class', 'bar series' + seriesIndex )
      .attr('class', function (d,barIndex) {
        return 'bar series' + seriesIndex + ' barIndex' + barIndex
      } )
      .attr('seriesIndex', seriesIndex)
